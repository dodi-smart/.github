...
| `.github/workflows/issue-triage.yml` | The classifier runs as steps of `gate`, between `agent-gate` and the picker, writes only above threshold, and never writes `Triage state` | The one-hosted-start rule and the verify step both depend on it. Below threshold the field stays empty and the agent decides; a value written on a guess is worse than none. |
...
| `.github/workflows/pr-review.yml` | `review-route` decides depth in the gate before the picker; `review-check` only annotates the verdict and never fails the job | A sensitive change hides in files the path regex does not name. A checker that can fail the run becomes a second reviewer with veto power and no accountability. |
