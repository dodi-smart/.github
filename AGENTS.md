...
| `.github/workflows/issue-triage.yml` | **DODI-00022** — issue-field ids resolve repo-scoped, and triage fails if it did not record `Triage state` | The token this job runs on is a repo-scoped GitHub App installation with no organisation permission, so `/orgs/{org}/issue-fields` returns 403 and always will. Resolve ids from `repository(owner,name){ issueFields }` — same fields, same ids, no org access. Never offer the org path as a fallback. The final verification step is not decoration: without it the job reported success with the fields unwritten, and the break surfaced days later as `issue-implement` refusing the issue. |
